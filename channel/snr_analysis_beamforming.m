...
function snr_analysis_beamforming
    ...
    % Define the MIMO channel matrix H_m
    H_m = [1.6330, 0.4082 - 0.7071i, 0.4082 + 0.7071i;
           1.1547, -0.5774 + 1i, -0.5774 - 1i;
           0, 0.7071 - 1.2247i, -0.7071 - 1.2247i];

    % Calculate beamforming weights (example using eigenvector)
    [V, ~] = eig(H_m' * H_m);
    w = V(:, 1); % Beamforming weight vector

    % Create random data indices for QAM modulation for each of the 3 transmit antennas
   % Transpose qam_modulated to ensure it is [3xN]
    qam_modulated = qammod(randi([0 M-1], total_samples, 3), M, 'UnitAveragePower', true);
    qam_modulated = transpose(qam_modulated);  % Make sure it is [3xN]

    for SNR_Bob_dB = SNR_values
        % Correct calls to the beamforming function with the properly formatted signal matrix
        qam_received_bob = mimo_channel_beamforming(qam_modulated, H_m, w);
        qam_received_eve = mimo_channel_beamforming(qam_modulated, H_m, w);

        % Add AWGN noise
        qam_received_bob = awgn(qam_received_bob, SNR_Bob_dB, 'measured');
        qam_received_eve = awgn(qam_received_eve, SNR_Eve_dB, 'measured');
